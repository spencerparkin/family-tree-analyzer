...
class RenderNode(object):
    ...
    def render_edges(self, draw, font, image_rect, world_rect):
        ...
        for key in self.sub_node_map:
            ...
            draw.line((point_a.x, point_a.y, point_b.x, point_b.y), fill=(0, 10, 100), width=4)
    ...
    def render_label_box(self, draw, font, image_rect, world_rect):
        ...
        for world_line in polygon.GenerateLineSegments():
            ...
            draw.line((image_line.point_a.x, image_line.point_a.y, image_line.point_b.x, image_line.point_b.y), fill=(255, 0, 0), width=4)
            point = world_rect.Map(self.label_box.Center(), image_rect)
        text_size = draw.textsize(text=self.person.name, font=font)
        draw.text((point.x - text_size[0] / 2, point.y - text_size[1] / 2), text=self.person.name, font=font, fill=(200, 200, 200))
    ...
    def calculate_graph_layout(self):
        total_width = 0.0
        margin = 0.1
        for key in self.sub_node_map:
            sub_node = self.sub_node_map[key]
            sub_node.calculate_graph_layout()
            sub_node.calculate_bounding_box()
            total_width += sub_node.bounding_box.Width() + 2.0 * margin

        location = Vector(-total_width / 2.0 + margin, -1.0)
        for key in self.sub_node_map:
            sub_node = self.sub_node_map[key]
        ...
        self.label_box.min_point = Vector(-0.5, -0.5)
        self.label_box.max_point = Vector(0.5, 0.5)
